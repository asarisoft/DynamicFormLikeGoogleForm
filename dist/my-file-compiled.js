...
class Form extends Component {
  ...
  populateData = () => {
    ...
    this.setState({
      sections: updatedSections,
      title: this.headerRef.state.title
    });
  };
  render() {
    const {
    ...
    return <FormContainer>
        ...
        {sections.map((section, sectionIndex) => <div key={sectionIndex}>
            <Section label={section.label} section={section} onAddField={() => this.addQuestionToSection(sectionIndex)} onToggleQustion={() => this.toggleQuestionsVisibility(sectionIndex)} />
            <div style={{
          display: section.isQuestionsVisible ? 'block' : 'none'
        }}>
              {section.questions.map((question, questionIndex) => <Question key={`${sectionIndex}${questionIndex}`} question={question} questionIndex={questionIndex} onClick={() => this.setActiveQuestion(sectionIndex, questionIndex)} onRemoveQuestion={() => this.removeQuestionFromSection(sectionIndex, questionIndex)} ref={ref => {
